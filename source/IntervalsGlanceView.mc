...
class IntervalsGlanceView extends WatchUi.GlanceView {
    ...
    hidden function drawMiniLoad(dc as Dc, ctl as Array, atl as Array,
        ...
        var eLo = pct ? ctlMin * 0.78 : ctlMin - 23.0;
        var eHi = pct ? ctlMax * 1.38 : ctlMax + 35.0;
        if (eLo < lo) { lo = eLo; }
        if (eHi > hi) { hi = eHi; }
        var pad = (hi - lo) * 0.03;
        lo -= pad;
        ...
        for (var x = x0; x <= x1; x += 3) {
            ...
            var prevY = y1;
            for (var z = 0; z < 5; z++) {
                var bandTop = z < 4 ? (pct ? c * MULS[z] : c - THRESHOLDS[z]) : hi;
    ...
    hidden function drawMiniForm(dc as Dc, ctl as Array, atl as Array,
        ...
        var pct = IntervalsSettings.formAsPercent();
        var eLo = pct ? -0.35 * ctlMax : -35.0;
        var eHi = pct ? 0.25 * ctlMax : 25.0;
        if (eLo < lo) { lo = eLo; }
        if (eHi > hi) { hi = eHi; }
        var pad = (hi - lo) * 0.03;
        lo -= pad;
        ...
        // In form space the zone edges run bottom-up: risk, optimal, grey,
        // fresh, transition. Absolute mode uses fixed TSB points; percent
        // mode scales the edges with that day's (interpolated) CTL.
        ...
        for (var x = x0; x <= x1; x += 3) {
            ...
            var prevY = y1;
            for (var z = 0; z < 5; z++) {
                var bandTop = z < 4 ? (pct ? c * edges[z] : edges[z]) : hi;
                var yTop = y1 - ((bandTop - lo) * scale).toNumber();
                if (yTop < y0) { yTop = y0; }
                if (yTop < prevY) {
                    dc.setColor(fills[z], Graphics.COLOR_TRANSPARENT);
