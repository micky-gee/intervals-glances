...
module IntervalsCharts {
    ...
    const ZONE_FILLS = [
        0x29293D,   // transition (form > +20%): slate
        0x14324D,   // fresh (+5..+20%): deep blue
        0x262626,   // grey zone (-10..+5%)
        0x103D24,   // optimal (-30..-10%): deep green
        0x451519    // high risk (< -30%): deep red
    ] as Array<Number>;
    ...
    function drawSeriesChart(dc as Dc, values as Array, color as Number,
            x0 as Number, x1 as Number, y0 as Number, y1 as Number) as Boolean {
        if (dc has :setAntiAlias) {
            dc.setAntiAlias(true);
        }
        var n = values.size();
            ...
            for (var i = 0; i < n; i++) {
                if (values[i] != null) {
                    dc.fillCircle(xAt(i, n, x0, x1),
                        yAt(values[i].toFloat(), minV, maxV, y0, y1), 4);
                }
            }
        }
    ...
    function drawRingChart(dc as Dc, values as Array, color as Number,
            cx as Number, cy as Number, rIn as Number, rOut as Number) as Boolean {
        if (dc has :setAntiAlias) {
            dc.setAntiAlias(true);
        }
        var n = values.size();
        ...
        for (var i = 0; i < n; i++) {
            ...
            var frac = (v.toFloat() - minV) / (maxV - minV);
            ...
            var sinv = Math.sin(rad);
            var r2 = r1 + span * frac;
            dc.setColor(i == lastIdx ? Graphics.COLOR_WHITE : color,
